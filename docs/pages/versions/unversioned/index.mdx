...
export default function App() {
  ...
  function addJob() {
    if (!pickUp.trim() || !dropOff.trim()) {
      Alert.alert("Error", "Pickup aur Drop-off donon bharen");
      return;
    }
    const newJob = {
  ...
  function markDone(id) {
    setJobs(jobs.map(j => j.id === id ? { ...j, status: "Done" } : j));
  }
  ...
  const renderItem = ({ item }) => (
    <View style={styles.job}>
      <View style={{flex:1}}>
        ...
        <Text>Drop: {item.dropOff}</Text>
        <Text>Status: {item.status}</Text>
      </View>
      <TouchableOpacity style={styles.btn} onPress={() => markDone(item.id)}>
        <Text style={{color:"#fff"}}>Mark Done</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Courier Sample App</Text>
      ...
      <TextInput
        ...
        style={styles.input}
      />
      <Button title="Add Delivery" onPress={addJob} />

      <Text style={{marginTop:16, fontWeight:"bold"}}>Deliveries</Text>
      <FlatList
        data={jobs}
        keyExtractor={(i) => i.id}
        renderItem={renderItem}
        ListEmptyComponent={<Text style={{marginTop:10}}>Koi delivery nahi hai.</Text>}
      />

      {/* Map Placeholder - Snack mein asli map nahi chalta, isliye placeholder */}
      <View style={styles.mapPlaceholder}>
        <Text>Map yahan aayega (Snack preview mein map limited hota hai)</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  job: { flexDirection:"row", padding:10, borderWidth:1, borderColor:"#ddd", borderRadius:8, marginTop:8, alignItems:"center" },
  title: { fontWeight:"bold" },
  btn: { backgroundColor:"#007AFF", padding:8, borderRadius:6 },
  mapPlaceholder: { height:140, marginTop:16, borderWidth:1, borderColor:"#aaa", alignItems:"center", justifyContent:"center", borderRadius:8 }
});
